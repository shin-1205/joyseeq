<footer class="l-footer">
  <div class="l-footer__inner">
    <div class="l-footer__left">
      <nav class="l-footer__nav">
        <ul class="l-footer__nav-list">
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>/#link_about">ABOUT</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_partner">PARTNER</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_support">SUPPORT</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_tarent">TALENT</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_flow">FLOW</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_qa">Q&A</a> </li>
          <li class="l-footer__nav-item"> <a href="<?php echo get_current_page_url(); ?>#link_movie">Movie</a> </li>
        </ul>
      </nav>
      <ul class="l-footer__sns">
        <li class="l-footer__sns-item"> <a href="twitter_jump.php" target="_blank" class="l-footer__sns-link"> <i
              class="l-footer__sns-icon"> <img src="./assets/images/common/icon_twitter.png" alt=""> </i> <span
              class="l-footer__sns-text"> Twitter </span> </a> </li>
        <li class="l-footer__sns-item"> <a href="instagram_jump.php" target="_blank" class="l-footer__sns-link"> <i
              class="l-footer__sns-icon"> <img src="./assets/images/common/icon_insta.png" alt=""> </i> <span
              class="l-footer__sns-text"> Instagram </span> </a> </li>
        <li class="l-footer__sns-item"> <a href="tiktok_jump.php" target="_blank" class="l-footer__sns-link"> <i
              class="l-footer__sns-icon"> <img src="./assets/images/common/icon_tiktok.png" alt=""> </i> <span
              class="l-footer__sns-text"> TikTok </span> </a> </li>
      </ul>
    </div>
    <!-- ./left -->
    <div class="l-footer__right">
      <p class="l-footer__logo"> <img src="./assets/images/common/logo_footer.png" alt=""> </p>
      <ul class="l-footer__sub_nav">
        <li class="l-footer__sub_nav-item"> &copy; JOYSEEQ inc. </li>
        <?php /*?><li class="l-footer__sub_nav-item"> 運営会社：株式会社ERAIZ </li><?php */?>
        <?php /*?><li class="l-footer__sub_nav-item"> <a href="./privacy.php">プライバシーポリシー</a> </li><?php */?>
      </ul>
    </div>
  </div>
</footer>
<a href="#" class="l-pagetop"> <img src="./assets/images/common/icon_page_top.png" alt=""> </a>
</div>
<!-- ./l-wrapper -->

<link rel="stylesheet" href="https://use.typekit.net/lcd1iym.css" media="print" onload="this.media='all'">
<link href="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css" rel="stylesheet" media="print"
  onload="this.media='all'">
<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.4/gsap.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.4/ScrollTrigger.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/lozad/dist/lozad.min.js" defer></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js" defer></script>
<script src="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js" defer></script>

<script src="./assets/script/scripts.js?<?php echo mt_rand(); ?>" defer></script>
<script src="./assets/script/scroll.js?<?php echo mt_rand(); ?>" defer></script>
<?php include_once("tag/end_body_tag.php") ?>
</body>

</html>